...
// NOTE: SD_CS = GPIO21. On the XIAO ESP32S3 Sense, GPIO21 is also tied
// to the onboard user LED, so SD writes visibly flash the onboard LED.
// This is a board-level hardware conflict, not controllable in firmware.
// ======================================================================
...
static bool recordingIrLedEnabled = true;
...
// Blocking stream/download handlers set this flag if MAINT goes LOW.
// The main loop performs the actual AP shutdown once the handler exits.
bool maintenanceShutdownRequested = false;
...
void updateIrLedOutput() {
  bool shouldBeOn =
    (recordingActive && recordingIrLedEnabled) ||
    irLedOverrideActive;
...
int batteryPercentFromMilliVolts(uint32_t mv) {
  ...
  static const size_t count = sizeof(table) / sizeof(table[0]);
...
void handleBatteryData() {
  ...
  String json = "{";
  json += "\"mv\":" + String(batMv) + ",";
  json += "\"pct\":" + String(batPct);
  json += "}";
...
bool writeCameraSettingsToSD() {
  ...
  f.println("# recording_ir_led: 1 = IR LED turns on while recording, 0 = recording does not use IR LED");
  ...
  f.print("recording_ir_led=");
  f.println(recordingIrLedEnabled ? 1 : 0);
...
bool loadCameraSettingsFromSD() {
  ...
  while (f.available()) {
    ...
    } else if (key == "recording_ir_led") {
      recordingIrLedEnabled = (val.toInt() != 0);
    }
  }
  ...
  logLine("Recording IR LED: " + String(recordingIrLedEnabled));
...
// ======================================================================
// AVI writer - MJPEG inside AVI container
// ======================================================================
...
void handleStream() {
  ...
  while (client.connected() && maintenanceActive) {
    if (!maintenancePinActive()) {
      logLine("Stream stopped because maintenance pin went LOW");
      maintenanceShutdownRequested = true;
      break;
    }
...
String filesListHTML() {
  ...
  html += "<p>";
  html += "<button onclick='syncLastId()'>";
  html += tr("Sync last_id.txt to highest existing video ID", "Synchronizovat last_id.txt podle nejvyššího existujícího ID videa");
  html += "</button> ";
  html += "<button onclick='downloadAllFiles()'>";
  html += tr("Download all", "Stáhnout vše");
  html += "</button> ";
  html += "<button onclick='deleteAllFiles()'>";
  html += tr("Delete all", "Smazat vše");
  html += "</button>";
  html += "</p>";

  html += "<p class='note'>";
  html += tr("Download all starts separate browser downloads one by one. Your browser may ask to allow multiple downloads.",
             "Stáhnout vše spouští samostatná stažení postupně. Prohlížeč může požádat o povolení více stažení.");
  html += "</p>";
  ...
  while (true) {
    ...
    if (!file.isDirectory() && name.endsWith(".avi")) {
      ...
      html += " <button onclick=\"downloadFile('";
      html += cleanName;
      html += "')\">";
      html += tr("Download", "Stáhnout");
      html += "</button>";
...
void handleDownload() {
  ...
  client.print("\"\r\n");
  client.print("Content-Length: ");
  client.print((uint32_t)file.size());
  client.print("\r\n");
  client.print("Cache-Control: no-cache\r\n");
  client.print("Connection: close\r\n\r\n");

  logLine("Download started: " + path);
  ...
  while (file.available()) {
    if (!client.connected()) {
      aborted = true;
      logLine("Download aborted by browser: " + path);
      break;
    }
    ...
    if (!maintenancePinActive()) {
      aborted = true;
      maintenanceShutdownRequested = true;
      logLine("Download aborted because maintenance pin went LOW: " + path);
      break;
    }
    ...
    if (written != n) {
      aborted = true;
      logLine("Download write failed or client aborted: " + path);
      break;
    }
  ...
  if (!aborted) {
    logLine("Download finished: " + path + ", bytes=" + String(totalSent));
  }
}
...
void handleDeleteAllFiles() {
  ...
  while (true) {
    ...
    if (!root) {
      server.send(500, "text/plain; charset=utf-8", tr("Failed to open SD root", "Nepodařilo se otevřít kořen SD karty"));
      return;
    }
    ...
    while (true) {
      ...
      String name = file.name();
      bool isAvi = !file.isDirectory() && name.endsWith(".avi");
      file.close();
    ...
    if (SD.remove(fileToDelete.c_str())) {
      deletedCount++;
      logLine("Deleted file: " + fileToDelete);
    } else {
      logLine("ERROR: Failed to delete file: " + fileToDelete);
      break;
    }
  }

  server.send(200, "text/plain; charset=utf-8",
              tr("Deleted AVI files: ", "Smazané AVI soubory: ") + String(deletedCount));
}
...
String cameraSettingsHTML() {
  ...
  html += "<select name='resolution' id='cameraResolution' onchange='updateCameraFpsLimit(true)'>";
  ...
  html += "<label><input type='checkbox' name='recording_ir_led' value='1'";
  if (recordingIrLedEnabled) html += " checked";
  html += "> ";
  html += tr("Turn IR LED on while recording", "Zapínat IR LED při nahrávání");
  html += "</label><br>";
...
void handleSaveCameraSettings() {
  ...
  recordingIrLedEnabled = server.hasArg("recording_ir_led");
...
void handleHomePage() {
  ...
  html += "nav button:disabled{opacity:.45;}";
  ...
  html += ".downloadHero{font-size:18px;font-weight:bold;}";
  html += "</style>";
  ...
  html += " / ";
  html += tr("recording use", "použití při nahrávání");
  html += ": ";
  html += recordingIrLedEnabled ? "<span class='ok'>ON</span>" : "<span>OFF</span>";
  html += " / ";
  ...
  html += ": <span id='batteryMv'>";
  html += String(batMv);
  html += "</span> mV / <span id='batteryPct'>";
  html += String(batPct);
  html += "</span>%</p>";

  html += "<p class='note'>";
  html += tr("Battery percentage is approximate and refreshes while the Status tab is open.",
             "Procenta baterie jsou pouze orientační a obnovují se při otevřené záložce Stav.");
  html += "</p>";
  ...
  html += "<p class='note'>";
  html += tr("This can only force the LED on. Recording can still turn it on if that camera setting is enabled. Leaving this tab turns override off.",
             "Toto umí LED pouze vynuceně zapnout. Nahrávání ji může zapnout, pokud je to povoleno v nastavení kamery. Při opuštění této záložky se vynucení vypne.");
  html += "</p>";
  ...
  html += "<section id='pageDownloading' class='page'>";
  html += "<div class='card'><h2>";
  html += tr("Downloading", "Stahování");
  html += "</h2>";
  html += "<p class='downloadHero' id='downloadMessage'>";
  html += tr("The download has been started.", "Stahování bylo spuštěno.");
  html += "</p>";
  html += "<p class='note'>";
  html += tr("The ESP32 web interface is intentionally locked on this screen during downloads, because the built-in web server handles long file transfers synchronously.",
             "Webové rozhraní ESP32 je během stahování záměrně uzamčeno na této obrazovce, protože vestavěný webserver obsluhuje dlouhé přenosy souborů synchronně.");
  html += "</p>";
  html += "<button onclick='returnFromDownloadScreen()'>";
  html += tr("Back to files", "Zpět k souborům");
  html += "</button>";
  html += "</div></section>";
  ...
  html += "let batteryTimer=null;";
  html += "let ledOverrideWanted=false;";
  html += "let downloadScreenActive=false;";
  ...
  html += "';";
  html += "const TXT_DELETE_ALL_Q='";
  html += tr("Delete all AVI files?", "Smazat všechny AVI soubory?");
  html += "';";
  html += "const TXT_DELETE_ALL_FAIL='";
  html += tr("Delete all failed", "Smazání všech souborů selhalo");
  html += "';";
  ...
  html += tr("Maximum FPS for this resolution: ", "Maximální FPS pro toto rozlišení: ");
  html += "';";
  html += "const TXT_NO_FILES='";
  html += tr("No AVI files found", "Nebyly nalezeny žádné AVI soubory");
  html += "';";
  html += "const TXT_PREPARE_DOWNLOAD_FAIL='";
  html += tr("Failed to prepare downloads", "Nepodařilo se připravit stahování");
  html += "';";
  html += "const TXT_DOWNLOADING='";
  html += tr("Downloading: ", "Stahování: ");
  html += "';";
  html += "const TXT_BATCH_DOWNLOADING='";
  html += tr("Starting batch download. Your browser may ask to allow multiple downloads.",
             "Spouští se hromadné stahování. Prohlížeč může požádat o povolení více stažení.");
  html += "';";
  ...
  html += "function setNavDisabled(disabled){document.querySelectorAll('nav button').forEach(b=>b.disabled=disabled);}";

  html += "function showPage(name){";
  html += "if(downloadScreenActive && name!=='downloading')return;";
  ...
  html += "const navBtn=document.getElementById('btn'+cap(name));if(navBtn)navBtn.classList.add('active');";
  ...
  html += "if(name==='status'){startBatteryAuto();}else{stopBatteryAuto();}";
  html += "}";
  ...
  html += "function deleteAllFiles(){";
  html += "stopStream();stopLedOverride();";
  html += "if(!confirm(TXT_DELETE_ALL_Q))return;";
  html += "fetch('/delete-all',{method:'POST'}).then(r=>r.text()).then(t=>{alert(t);refreshFiles();}).catch(()=>{alert(TXT_DELETE_ALL_FAIL);});";
  html += "}";
  ...
  html += "function refreshBattery(){";
  html += "fetch('/battery-data?t='+Date.now()).then(r=>r.json()).then(j=>{";
  html += "const mv=document.getElementById('batteryMv');";
  html += "const pct=document.getElementById('batteryPct');";
  html += "if(mv)mv.innerText=j.mv;";
  html += "if(pct)pct.innerText=j.pct;";
  html += "}).catch(()=>{});";
  html += "}";

  html += "function startBatteryAuto(){if(batteryTimer)return;refreshBattery();batteryTimer=setInterval(refreshBattery,3000);}";
  html += "function stopBatteryAuto(){if(batteryTimer){clearInterval(batteryTimer);batteryTimer=null;}}";
  ...
  html += "function updateCameraFpsLimit(setToMax=false){";
  ...
  html += "if(setToMax){";
  html += "  fps.value = max;";
  html += "} else if(parseInt(fps.value) > max){";
  html += "  fps.value = max;";
  html += "}";
  html += "if(note){";
  html += "  note.innerText = TXT_MAX_FPS + max;";
  html += "}";
  html += "}";
  ...
  html += "function showDownloadScreen(text){";
  html += "downloadScreenActive=true;";
  html += "stopStream();stopLedOverride();stopLogAuto();stopBatteryAuto();";
  html += "setNavDisabled(true);";
  html += "document.querySelectorAll('.page').forEach(p=>p.classList.remove('active'));";
  html += "document.getElementById('pageDownloading').classList.add('active');";
  html += "const msg=document.getElementById('downloadMessage');if(msg)msg.innerText=text;";
  html += "}";

  html += "function returnFromDownloadScreen(){";
  html += "downloadScreenActive=false;";
  html += "setNavDisabled(false);";
  html += "showPage('files');";
  html += "}";

  html += "function downloadFile(name){";
  html += "showDownloadScreen(TXT_DOWNLOADING+name);";
  html += "const old=document.getElementById('downloadFrame');if(old)old.remove();";
  html += "const iframe=document.createElement('iframe');";
  html += "iframe.id='downloadFrame';iframe.style.display='none';iframe.src='/download?name='+encodeURIComponent(name)+'&t='+Date.now();";
  html += "document.body.appendChild(iframe);";
  html += "}";

  html += "function downloadAllFiles(){";
  html += "stopStream();stopLedOverride();";
  html += "fetch('/files-json?t='+Date.now()).then(r=>r.json()).then(files=>{";
  html += "if(!files.length){alert(TXT_NO_FILES);return;}";
  html += "showDownloadScreen(TXT_BATCH_DOWNLOADING);";
  html += "files.forEach((name,i)=>{";
  html += "setTimeout(()=>{";
  html += "const a=document.createElement('a');";
  html += "a.href='/download?name='+encodeURIComponent(name)+'&t='+Date.now();a.download=name;document.body.appendChild(a);a.click();a.remove();";
  html += "},i*2500);";
  html += "});";
  html += "}).catch(()=>{alert(TXT_PREPARE_DOWNLOAD_FAIL);});";
  html += "}";
...
void registerCommonRoutes(bool errorOnly) {
  ...
  server.on("/files-json", HTTP_GET, handleFilesJson);
  ...
  server.on("/battery-data", HTTP_GET, handleBatteryData);
...
void startMaintenanceServer() {
  ...
  maintenanceShutdownRequested = false;
...
// ======================================================================
// Deep sleep
// Wake by PIR GPIO4 OR maintenance GPIO3, both active HIGH.
...
void setup() {
  ...
  while (true) {
    ...
    if (maintenanceActive) {
      ...
      if (maintenanceShutdownRequested) {
        stopMaintenanceServer();
      }
    }
